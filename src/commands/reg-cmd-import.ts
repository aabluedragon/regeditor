...
import { applyParamsModifier, execFileUtil, findCommonErrorInTrimmedStdErr, isKnownWineDriverStderrOrWindows, optionalElevateCmdCall } from "../utils";
        ...
        return newStoppable<RegImportCmdResult>((resolve, reject, setStopper) => {
            ...
            const proc = execFileUtil(params, {
                ...
                onExit() {
                    ...
                    if (trimmedStdErr.length && trimmedStdErr !== 'The operation completed successfully.' && !isKnownWineDriverStderrOrWindows(stderrStr)) return reject(new RegErrorGeneral(stderrStr)); // REG IMPORT writes a success message into stderr.
                    resolve({ cmds: [params] });
                },
            }, elevated);
