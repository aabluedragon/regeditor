            ...
            const proc = execFileUtil(params, {
                ...
                async onExit(code) {
                    ...
                    if (trimmedStdErr.length && trimmedStdErr !== 'The operation completed successfully.' && !isKnownWineDriverStderrOrFirstTimeWineRun(stderrStr)) { // REG IMPORT writes a success message into stderr.
                        const err = await nonEnglish_REG_regFileMissingOrAccessDenied(code, fileName);
 ...
 */
async function nonEnglish_REG_regFileMissingOrAccessDenied(exitCode: number | null, regFilePath:string): Promise<'missing' | 'accessDenied' | null> {
